...
import V2FactoryArtifact from '../../config/constants/UniswapV2Factory.json';
import V2RouterArtifact from '../../config/constants/UniswapV2Router02.json';
import { V2FACTORY_ADDRESS, V2ROUTER_ADDRESS, zero_address } from '../../config/types';

import { INPUT, OUTPUT } from "../../config/types";
...
const Swap = () => {
    ...
    const { connect,status,reset } = useWallet();
    ...
    useEffect(()=>{
        console.log("check status",account)
    },[account])
    ...
    return (
        <>
            <div className="flex col t-swap space-between self-center">
                <div className="flex t-swap-setting space-between align-center">
                    <div className="flex t-swap-label">
                        SWAP
                    </div>
                    <div className="flex t-open-setting">
                        <SmoothButton label={<FiSettings />} style={{ borderRadius: "50%", width: "50px", height: "50px", outline: "none" }} onClick={() => setShowSettingModal(true)} />
                    </div>
                </div>
                <div className="flex t-token-input justify-center">
                ...
                <div className="flex t-confirm-container justify-center">
                    {
                        account ? <GoldButton label={"Confirm Swap"} onClick={ wrappingCondition ? swapWithETH : swapTokenforToken} /> : <GoldButton label={"Connect Wallet NEW"} onClick={() => activate(injected)} />
                    }
                </div>
            </div>
